...
- `docker/broker/` — Build-Kontext des deploybaren Broker-Images (`Dockerfile` + `prepareDocker`-Staging); in `gradle.properties` via `bnd_exclude` vom bnd-Build ausgenommen. Gebaut und gepusht vom zentralen `reusable-container` aus `eclipse-fennec/.github` (siehe [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md))
