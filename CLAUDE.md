...
- `org.eclipse.fennec.services.rsa` / `rsa.distribution.rest` / `rsa.discovery.rest` / `rsa.topology` — OSGi Remote Service Admin auf DDSR (#24): Kern mit zwei SPIs (`FlavorDistribution`, `ServiceDiscovery`), je Flavor ein Provider über RSA-Config-Types, lokale Service-Registry auf der EObject-Registry; `examples.rsa` ist der Beweis (ein simpler OSGi-Service, exportiert ohne Vertragsdokument)
