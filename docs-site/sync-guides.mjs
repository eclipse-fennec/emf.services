...
// (see guides.mjs) — internal dev docs (architecture, test plans, security
...
// on GitHub, not published). Passed by CI as github.ref_name. The local default is
// `snapshot`: it matches the VitePress config default, and `main` does not exist in
// this repository yet — defaulting to it would produce 404 links in local builds.
const branch = process.env.DOCS_BRANCH || 'snapshot';
const blobBase = `https://github.com/eclipse-fennec/emf.services/blob/${branch}/docs`;
